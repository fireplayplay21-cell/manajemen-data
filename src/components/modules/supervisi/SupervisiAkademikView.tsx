...
export const SupervisiAkademikView: React.FC = () => {
  ...
  // Filter list: If user is a teacher and privacy is active, ONLY show documents that match the logged-in teacher
  const safeFormulirList = (formulirSupervisiList || []).filter(item => {
    if (isGuru && supervisiPrivateForGuru) {
      return isTeacherSupervisiMatch(item, currentUser);
    }
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6 animate-in fade-in duration-200">
        {/* Guru Privacy Notification Banner */}
        {isGuru && (
          <div className="p-4 bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-emerald-950 shadow-xs">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1">
                    ...
                    <span>Hanya Dokumen Anda</span>
                  </span>
                </div>
                <p className="text-xs text-emerald-800 mt-1 leading-relaxed">
                  Sesuai kebijakan kerahasiaan evaluasi kinerja pendidik, Anda hanya dapat melihat instrumen hasil supervisi pembelajaran atas nama akun Anda (<strong className="text-emerald-950">{currentUser.nama}</strong> • NIP: <span className="font-mono font-bold text-emerald-900">{currentUser.nip || '-'}</span>). Dokumen rekan guru lain dirahasiakan.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0 text-xs font-bold text-emerald-900 bg-white/80 backdrop-blur-xs px-3.5 py-2 rounded-xl border border-emerald-300 shadow-2xs">
              <UserCheck className="w-4 h-4 text-emerald-700" />
              <span>Dokumen Anda: {safeFormulirList.length}</span>
            </div>
          </div>
        )}

        {/* Reference Info Card (Shown for KS & Admin) */}
        {!isGuru && (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder={isGuru ? "Cari dalam dokumen supervisi Anda..." : "Cari nama guru, mata pelajaran, kelas, atau supervisor..."}
        ...
        {filteredFormulirList.length === 0 ? (
          isGuru ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-500">
              ...
              <p className="font-bold text-slate-800 text-sm">Belum Ada Dokumen Supervisi untuk Akun Anda</p>
              <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto leading-relaxed">
                Dokumen hasil supervisi akademik atas nama <strong className="text-slate-700">{currentUser.nama}</strong> (NIP: {currentUser.nip || '-'}) belum diterbitkan oleh Kepala Sekolah / Tim Penilai. Jika jadwal observasi pembelajaran kelas Anda telah dilaksanakan, silakan hubungi Kepala Sekolah.
              </p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-500">
